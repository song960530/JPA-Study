...
### 값 타입
  ...
  - 불변 객체로 만드는 것이 안전하며, 공유하지 않는게 좋다
  - 생명주기를 엔티티에 의존한다
...
![컬렉션 값 타입](https://user-images.githubusercontent.com/52727315/152814166-b5fb85b6-bf04-4a44-a9d3-bf49bd97cc99.png)
   ...
   -  **✨실무에서 상황에 따라 컬렉션 값 타입 대신 엔티티로 만들어 일대다 관계매핑을 고려  
(가능하면 값 타입은 지양)✨ **

------------------------------------------------------------------------------------------------------------------------------------------------------------

### JPQL
  - SQL을 추상화한 객체 지향 쿼리 언어, 특정 데이터베이스 SQL에 의존하지않음
  - 테이블을 대상으로 하는 SQL과 달리 엔티티를 대상으로 함

### Criteria
  - 문자가 아닌 자바코드로 JPQL을 작성할 수 있음
  - JPA 공식 기능
  - **🔥단점으로 너무 복잡하고 실용성이 없어 유지보수에도 힘이듦.. 그래서 실부에선 잘 사용하지 않음🔥**
  - **✨Criteria 대신 QueryDSL 사용 권장 ✨**
  
### QueryDSL
  - 문자가 아닌 자바코드로 JPQL을 작성할 수 있는 오픈라이브러리
  - 동적 쿼리 작성이 편리함
  - 단순하며 쉬움
  - 컴파일 레벨에서 오류를 찾을 수 있음
  - **✨실무에서 사용 권장✨**
...
### ✨페치 조인(fetch join)✨ 실무에서 정말 중요
  ...
  - 여러 테이블을 조인해서 결과를 도출해야한다면 페치 조인보단 일반 조인을 사용하여 DTO로 반환하는것이 효과적이다
...
### 페치 조인의 특징과 한계
  - 특징
    - 연관된 엔티티를 한번에 조회가 가능하므로 성능이 최적화된다 (네트워크를 여러번 태우지 않음)
    - 엔티티에 직접 적용하는 로딩전략보다 우선시된다
  - 한계
    - 페치조인 대상에는 별칭(alias)를 줄 수 없다. (하이버네이트에선 제공하지만 가급적 사용X)
    - 둘 이상의 컬렉션은 페치 조인할 수 없다
    - 컬렉션을 페치조인하면 페이징 API를 사용할 수 있다
      - 일대일, 다대일은 가능

------------------------------------------------------------------------------------------------------------------------------------------------------------

### @NamedQuery
  - 미리 JPQL을 정의 하여 이름을 부여하는 방식
  - 어노테이션, XML에 정의 가능
  - 재사용성이 있으며, 애플리케이선 로딩 시점에 쿼리를 검증한다
  - 다만 엔티티가 많이 더러워질 수 있고, 이후에 Spring Data JPA에서 다른 대안책이 있어서 잘 사용하진 않는다

------------------------------------------------------------------------------------------------------------------------------------------------------------

### 벌크 연산
  - executeUpdate(): 쿼리 한번으로 여러 로우를 변경
  - Update, Delete, Insert(insert into... select... 하이버네이트에서만 지원)
  - 주의사항
    - 영속성 컨텍스트를 무시하고 바로 DB에 때려넣기 때문에 벌크 연산을 먼저 실행하거나, 실행 후 영속성 컨텍스트를 초기화 하고 다시 Select 해줘야함
```java
Member findMember = em.find(Member.class,member.getId());

em.createQuery("update Member m set m.age = 30").executeUpdate();

findMember.getAge() // 기존에 영속성 컨텍스트에 캐시되어있던 나이로 나옴
```
------------------------------------------------------------------------------------------------------------------------------------------------------------
